...
//                         //new added motor
//                         $('.walk-right').css('background-image', 'url("images/almoto4.png")');
...
var lastScrollTop = 0,
    isFirstTimeSea = true,
    isFirstTimeSand = true,
    isFirstTimeFinal = true,
    isFirstTimeResume = true,
    isFirstTimeSocial = true,
    isFirstTimebackSea = false,
    isFirstTimebackSand = false;
...
window.addEventListener("load", function () {
    setTimeout(() => $(loader).animate({ opacity: 0 }, 1000), 1000);
    setTimeout(() => $(loader).hide(), 2000);
    setTimeout(() => $("body").css("overflow-y", "scroll"), 2000);
});

// =====================
// DIMENSIONS
// =====================
var height_device = $win.height(),
    document_height = $doc.height() - height_device,
    window_width = $win.width() - (part1.clientHeight + part2.clientHeight + part3.clientHeight);

// =====================
// INITIAL POSITIONS
// =====================
$(start).css({ left: 300, bottom: 370 });
$(house).css({ left: 700 });
$(school).css({ left: 1600 });
$(education).css({ left: 1900, bottom: 200 });
$(skill_post).css({ left: 2800 });

$me.css({
    left: -me.clientWidth / 2 + $win.width() / 2,
    bottom: 103,
    zIndex: 9998
});
...
// =====================
function dive_in_the_sea() {
    $ground.stop(true).animate({ bottom: -25 }, 600);
    $ground3.stop(true).animate({ bottom: -25 }, 600);
    $blue.stop(true).animate({ bottom: -25 }, 600);

    $me.stop(true).animate({ bottom: 0 }, 600)
        .css({ border: "3px solid white", borderRadius: 50, background: "rgba(0,0,0,.5)" });

    $(bubble).show();
}

function go_in_part3() {
    $(bubble).hide();
    $me.css({ border: 0, background: "" }).animate({ bottom: 103 }, 200);

    $ground.animate({ bottom: 103 }, 600);
    $ground3.animate({ bottom: 103 - ground.clientHeight }, 200);
}
...
function in_part1() {
    ...
    $ground.animate({ bottom: 103 - ground.clientHeight }, 200);
    $blue.animate({ bottom: 103 - ground.clientHeight }, 200);
    $ground2.animate({ bottom: 53 }, 200);
}
